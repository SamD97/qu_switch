...
for i in range(no_f):
    ...
    for rep in pd.unique(df.r):
        ...
        plt.plot(frem.g, mfit, label=next(rplbl))
    ...
    plt.yticks(np.arange(27)+0.5,names[2:],fontsize=10,rotation=15)
    plt.xticks(np.arange(10)+0.5,repsn,rotation=15)
    plt.ylabel('Genotypes')
    plt.xlabel('Replicates')
    plt.suptitle('Population size = ${}*10^{}$'.format(str(ccap)[0],len(str(ccap)[1:-2])), y=0.99)
